...
The minor findings of the task reviews, which the final whole-branch review triaged as able to wait, and what it and the re-review of its fix wave found beside them. The maintainer decides which, if any, must be fixed before merging; the last one is raised with them.
...
- Final review: the vocabulary state is photographed with its list closed; a panel opened from the canvas by keyboard leaves the focus on the arrow; a `definition-mismatch` is reported at both files, so the panel lists its sentence twice (visible in the screenshots); the canvas keeps its viewport when a panel narrows it, so part of the project is out of view until Fit.
- Re-review of the fix wave: a list React Aria flips above the field sits flush on it (`offset={0}` relies on a `margin-top`); and **a file saved with an error closes the panel of a variable only that file declares, with a false "no longer declared" banner, and the panel does not come back when the file is fixed** - `GET /api/variable` answers the same `404 not-found` for a name no file declares, for one whose only declaring file did not load, and for a revision without an index.
...
Rulings 14 to 29 were taken executing Tasks 4 to 11 and the final review, from 2026-09-18 20:09:
