...
* **A browser interface, as a preview.**  `ddd gui` serves a browser interface over one
  ...
  compiled pages, and so does the wheel ci builds for every branch it runs on.  `--host` can
  also serve it from a container: published on the host's loopback alone, with the token in
  the address as the only guard beyond it.
