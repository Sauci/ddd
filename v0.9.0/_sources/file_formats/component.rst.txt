...
.. note::
   ...
   Seven keys of the block sit outside that. Five are refused on a consumer altogether -
   ...
   the variable, not by one that reads it. Two more are compared by nobody: ``export``,
   because any component may ask for an object to reach the a2l and asking wins over
   declining, and ``description``, where the producer's text is simply the one that is used.
