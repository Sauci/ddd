...
* **A `definition-mismatch` finding now carries a button for each key its declarations
  ...
  that value outward, and the reader never chooses a direction.  Previewed first, it
  ...
  finding, still settles any value the reader chooses.  Nor where only `kind` disagrees,
  the one key no edit may carry between declarations, or where some declaration cannot
  take the value.
