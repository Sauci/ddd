...
- Known failures, and only these: on Windows `tests/test_lsp.py::TestSymlinkedWorkspace::test_a_document_opened_through_a_symlink_is_covered_by_its_build` (no symlink privilege); on the Linux PC's Python 3.14 venv `tests/test_hardening.py::TestInputTheToolMustSurvive::test_json_nested_beyond_what_python_can_read`, which also leaves the two lines of the `RecursionError` handler in `src/ddd/loading.py` uncovered. CI runs 3.12 and 3.13, where both pass.
...
`docker compose build gui && docker compose up gui` on the Linux PC, or `ddd gui examples/demo/demo.ddd.json` where Node built the pages: make ValueA disagree from outside, resolve it from the component page and from the canvas, and take screenshots of the panel, the picker open and Show changes, for the pull request. Leave `examples/demo` as it was, and if the edit was made from the container, put the file's owner and mode back.
